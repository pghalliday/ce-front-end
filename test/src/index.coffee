...
describe 'ce-front-end', ->
  describe 'on start', ->
    ...
    it 'should override parameters from a file with parameters from the command line', (done) ->
      ...
      childDaemon = new ChildDaemon 'node', ['lib/src/index.js', '--config', 'test/support/config.json', '--port', '3003', '--ce-operation-hub', 'tcp://127.0.0.1:4003'], new RegExp 'ce-front-end started'
      childDaemon.start (error, matched) =>
        expect(error).to.not.be.ok
